...
def test_db_exists(db_path):
    ...
    assert db_path.is_file(), "File database not found"
...
def table_names(db_conn):
    name_tuples = db_conn.execute("SELECT name FROM sqlite_master \
...
def test_employee_table_exists(table_names):
    ...
    assert "employee" in table_names, "Employees table not found"
...
def test_team_table_exists(table_names):
    ...
    # is in the table_names list
    assert "team" in table_names, "Teams table not found"
...
def test_employee_events_table_exists(table_names):
    ...
    # is in the table_names list
    assert "employee_events" in table_names, "Employee_events table not found"
